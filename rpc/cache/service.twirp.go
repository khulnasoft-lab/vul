...
func (c *cacheProtobufClient) PutArtifact(ctx context.Context, in *PutArtifactRequest) (*google_protobuf1.Empty, error) {
	ctx = ctxsetters.WithPackageName(ctx, "vul.cache.v1")
...
const CachePathPrefix = "/twirp/vul.cache.v1.Cache/"
...
func (s *cacheServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	...
	switch req.URL.Path {
	case "/twirp/vul.cache.v1.Cache/PutArtifact":
		s.servePutArtifact(ctx, resp, req)
		return
	case "/twirp/vul.cache.v1.Cache/PutBlob":
		s.servePutBlob(ctx, resp, req)
		return
	case "/twirp/vul.cache.v1.Cache/MissingBlobs":
