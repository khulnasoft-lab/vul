...
func buildLdflags() (string, error) {
	...
	return fmt.Sprintf("-s -w -X=github.com/khulnasoft-lab/vul/pkg/version.ver=%s", ver), nil
}
...
func Protoc() error {
	...
	if err := sh.RunV("bash", "-c", "docker build -t vul-protoc - < Dockerfile.protoc"); err != nil {
		return err
	}
	return sh.Run("docker", "run", "--rm", "-it", "--platform", "linux/x86_64", "-v", "${PWD}:/app", "-w", "/app", "vul-protoc", "mage", "protoc")
}
...
func Install() error {
	...
	return sh.RunWith(ENV, "go", "install", "-ldflags", ldflags, filepath.Join(wd, "cmd", "vul"))
}
