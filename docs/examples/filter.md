...
By default, `Vul` also detects unpatched/unfixed vulnerabilities. This means you can't fix these vulnerabilities even if you update all packages.
...
Vul supports Open Policy Agent (OPA) to filter vulnerabilities. You can specify a Rego file with `--ignore-policy` option.
...
```bash
$ vul image --ignore-policy contrib/example_filter/basic.rego centos:7
```
...
[helper]: https://github.com/khulnasoft-lab/vul/tree/main/pkg/vulnerability/module.go
[policy]: https://github.com/khulnasoft-lab/vul/tree/main/contrib/example_policy
