...
```
$ vul image -f table golang:1.12-alpine
```

## JSON

```
$ vul image -f json -o results.json golang:1.12-alpine
```
...
[
  ...
  {
    "Target": "vul-ci-test (alpine 3.7.1)",
    "Vulnerabilities": [
      {
...
{% raw %}
```
$ vul image --format template --template "{{ range . }} {{ .Target }} {{ end }}" golang:1.12-alpine
```
{% endraw %}
...
{% raw %}
```
$ vul image --format template --template '{{- $critical := 0 }}{{- $high := 0 }}{{- range . }}{{- range .Vulnerabilities }}{{- if  eq .Severity "CRITICAL" }}{{- $critical = add $critical 1 }}{{- end }}{{- if  eq .Severity "HIGH" }}{{- $high = add $high 1 }}{{- end }}{{- end }}{{- end }}Critical: {{ $critical }}, High: {{ $high }}' golang:1.12-alpine
```
{% endraw %}
...
```
$ vul image --format template --template "@/path/to/template" golang:1.12-alpine
```
...
```
$ vul image --format template --template "@contrib/junit.tpl" -o junit-report.xml  golang:1.12-alpine
```

#### SARIF
In the following example using the template `sarif.tpl` [Sarif][sarif] can be generated.
```
$ vul image --format template --template "@contrib/sarif.tpl" -o report.sarif  golang:1.12-alpine
```
This SARIF format can be uploaded to GitHub code scanning results, and there is a [Vul GitHub Action][action] for automating this process.
...
[asff]: https://github.com/khulnasoft-lab/vul/tree/main/docs/integration/security-hub.md
