...
```bash
$ vul image --ignore-status affected,fixed ruby:2.4.0
```
...
```bash
$ vul image --ignore-unfixed ruby:2.4.0
```
...
```bash
$ vul image --severity HIGH,CRITICAL ruby:2.4.0
```
...
```bash
vul conf --severity HIGH,CRITICAL examples/misconf/mixed
```
...
Vul supports the [.vulignore](#vulignore) and [.vulignore.yaml](#vulignoreyaml) ignore files.
...
```bash
$ vul image python:3.4-alpine3.9
```
...
When the extension of the specified ignore file is either `.yml` or `.yaml`, Vul will load the file as YAML.
For the `.vulignore.yaml` file, you can set ignored IDs separately for `vulnerabilities`, `misconfigurations`, `secrets`, or `licenses`[^1].
...
```bash
$ vul image --ignorefile ./.vulignore.yaml python:3.9.16-alpine3.16
```
...
2023-08-31T11:10:27.155+0600	INFO	Please see also https://khulnasoft-lab.github.io/vul/dev/docs/scanner/secret/#recommendation for faster secret detection
...
```bash
$ vul image --vuln-type os ruby:2.4.0
```
...
```bash
$ vul image --ignore-policy contrib/example_policy/basic.rego centos:7
```
...
[helper]: https://github.com/khulnasoft-lab/vul/tree/{{ git.tag }}/pkg/result/module.go
[policy]: https://github.com/khulnasoft-lab/vul/tree/{{ git.tag }}/contrib/example_policy
...
In cases where vul can detect comments of a specific format immediately adjacent to resource definitions, it is possible to filter/ignore findings from a single point of resource definition (in contrast to `.vulignore`, which has a directory-wide scope on all of the files scanned).

The format for these comments is `vul:ignore:<Vulnerability ID>` immediately following the format-specific line-comment token.

For example, to filter a Vulnerability ID "AVD-GCP-0051" in a Terraform HCL file:

```terraform
#vul:ignore:AVD-GCP-0051
...
[^1]: license name is used as id for `.vulignore.yaml` files
