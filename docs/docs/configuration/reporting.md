...
```sh
$ vul fs --severity HIGH,CRITICAL --dependency-tree /path/to/your_node_project
...
```
$ vul image --format github -o report.gsbom alpine
```
...
```shell
$ vul image --format json -o result.json --list-all-pkgs debian:11
...
$ vul convert --format table --severity CRITICAL result.json
```

!!! note
    JSON reports from "vul aws" and "vul k8s" are not yet supported.
