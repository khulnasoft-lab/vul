...
```
$ vul image --server http://localhost:8080 alpine:3.10
```
...
```shell
$ vul fs --server http://localhost:8080 --severity CRITICAL ./integration/testdata/fixtures/fs/pom/
```
**Note**: It's important to specify the protocol (http or https).
<details>
<summary>Result</summary>
...
```shell
$ vul rootfs --server http://localhost:8080 --severity CRITICAL /tmp/rootfs
```
**Note**: It's important to specify the protocol (http or https).
<details>
<summary>Result</summary>
...
```
$ vul image --server http://localhost:8080 --token dummy alpine:3.10
```
