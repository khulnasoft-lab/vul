...
```shell
$ vul image --scanners vuln [YOUR_IMAGE_NAME]
```
...
```
$ vul image --scanners config [YOUR_IMAGE_NAME]
```
...
```shell
$ vul image --scanners license [YOUR_IMAGE_NAME]
```
...
```
$ vul image --image-config-scanners config [YOUR_IMAGE_NAME]
```
...
```
$ vul image --scanners none --image-config-scanners config alpine:3.17.0
```
...
```shell
$ vul image --image-config-scanners secret [YOUR_IMAGE_NAME]
```
...
```shell
$ vul image --scanners none --image-config-scanners secret vuln-image
```
...
```bash
vul image --image-src podman,containerd alpine:3.7.3
```
...
```
# Referenced by tag
$ vul image --input /path/to/alpine:3.15

# Referenced by digest
$ vul image --input /path/to/alpine@sha256:82389ea44e50c696aba18393b168a833929506f5b29b9d75eb817acceb6d54ba
```
...
```
$ vul image --compliance docker-cis [YOUR_IMAGE_NAME]
```
...
```
$ vul image --platform=os/architecture [YOUR_IMAGE_NAME]
```

For example:

```
$ vul image --platform=linux/arm alpine:3.16.1
```
...
```shell
$ vul image --docker-host tcp://127.0.0.1:2375 YOUR_IMAGE
```
