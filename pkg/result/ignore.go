...
// IgnoreConfig represents the structure of .vulignore.yaml.
...
func getIgnoredFindings(ignoreFile string) (IgnoreConfig, error) {
	...
	if _, err := os.Stat(ignoreFile); errors.Is(err, fs.ErrNotExist) {
		// .vulignore doesn't necessarily exist
		...
		// IDs in .vulignore are treated as IDs for all scanners
...
func parseIgnore(ignoreFile string) (IgnoreFindings, error) {
	...
	for scanner.Scan() {
		...
		if len(fields) > 1 {
			...
			if err != nil {
				log.Logger.Warnf("Error while parsing expiration date in .vulignore file: %s", err)
				continue
			}
		}
