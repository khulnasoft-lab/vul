...
import (
	...
	_ "github.com/khulnasoft-lab/vul/pkg/fanal/handler/all"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.analyzerType == analyzer.TypeJar {
				// init java-vul-db with skip update
				javadb.Init("./language/java/jar/testdata", "ghcr.io/khulnasoft-lab/vul-java-db", true, false, types.RegistryOptions{Insecure: false})
			}
