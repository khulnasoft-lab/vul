...
func (c *BOM) UnmarshalJSON(b []byte) error {
	...
	if bom.Metadata != nil {
		...
		if bom.Metadata.Component != nil {
			metadata.Component = toVulCdxComponent(lo.FromPtr(bom.Metadata.Component))
		}
	}

	var components []ftypes.Component
	for _, component := range lo.FromPtr(bom.Components) {
		components = append(components, toVulCdxComponent(component))
	}
...
func (c *BOM) parseSBOM(bom *cdx.BOM) error {
	...
	for bomRef := range c.dependencies {
		...
		case cdx.ComponentTypeApplication: // It would be a lock file in a CycloneDX report generated by Vul
...
func toPackage(component cdx.Component) (bool, ftypes.TargetType, *ftypes.Package, error) {
	...
	pkg := p.Package()
	// Vul's marshall loses case-sensitivity in PURL used in SBOM for packages (Go, Npm, PyPI),
...
func toVulCdxComponent(component cdx.Component) ftypes.Component {
