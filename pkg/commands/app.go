...
import (
	...
	k8scommands "github.com/khulnasoft-lab/vul/pkg/k8s/commands"
...
func NewRootCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "vul [global flags] command [flags] target",
...
func NewImageCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ vul image --severity HIGH,CRITICAL alpine:3.15

  # Ignore unfixed/unpatched vulnerabilities
  $ vul image --ignore-unfixed alpine:3.15

  # Scan a container image in client mode
  $ vul image --server http://127.0.0.1:4954 alpine:latest

  # Generate json result
  $ vul image --format json --output result.json alpine:3.15

  # Generate a report in the CycloneDX format
  $ vul image --format cyclonedx --output result.cdx alpine:3.15`,
...
func NewFilesystemCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  # Scan a single file
  $ vul fs ./vul-ci-test/Pipfile.lock`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewClientCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			log.Logger.Warn("'client' subcommand is deprecated now. See https://github.com/khulnasoft-lab/vul/discussions/2119")
...
func NewKubernetesCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  # resources scanning:
  $ vul k8s --report=summary deploy
  $ vul k8s --namespace=kube-system --report=summary deploy,configmaps

  # resource scanning:
  $ vul k8s deployment/orion
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
`, strings.Join(services, "\n- ")),
  ...
  $ vul aws --region us-east-1 --service s3

  # limit scan to multiple services:
  $ vul aws --region us-east-1 --service s3 --service ec2

  # force refresh of cache for fresh results
  $ vul aws --region us-east-1 --update-cache
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewVMCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ vul vm --scanners vuln ami:${your_ami_id}

  # Scan your AWS EBS snapshot
  $ vul vm ebs:${your_ebs_snapshot_id}
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewSBOMCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ vul sbom /path/to/report.cdx

  # Scan CycloneDX-type attestation and show the result in tables
  $ vul sbom /path/to/report.cdx.intoto.jsonl
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
