...
import (
	...
	tcache "github.com/khulnasoft-lab/vul/pkg/cache"
	...
	"github.com/khulnasoft-lab/vul/pkg/javadb"
...
func Run(ctx context.Context, opts flag.Options, targetKind TargetKind) (err error) {
	...
	if opts.GenerateDefaultConfig {
		log.Logger.Info("Writing the default config to vul-default.yaml...")
		return viper.SafeWriteConfigAs("vul-default.yaml")
	}
...
func initScannerConfig(opts flag.Options, cacheClient cache.Cache) (ScannerConfig, types.ScanOptions, error) {
	...
	if opts.Scanners.Enabled(types.SecretScanner) {
		...
		log.Logger.Infof("Please see also https://khulnasoft-lab.github.io/vul/%s/docs/scanner/secret/#recommendation for faster secret detection", ver)
	} else {
